...
struct _cuda_device_id {
  ...
  unsigned num_shader() const { return m_gpgpu[0]->get_config().num_shader(); }
  ...
  const struct cudaDeviceProp *get_prop() const { return m_gpgpu[0]->get_prop(); }
...
struct CUctx_st {
  CUctx_st(_cuda_device_id *gpu, gpgpu_context * ctx) {
    m_gpu = gpu;
